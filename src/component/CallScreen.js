...
import {
  ...
  Text,
  ActivityIndicator,
} from "react-native"; // Đảm bảo chỉ sử dụng các component của react-native
...
const CallScreen = ({
  ...
  const startCall = useCallback(async () => {
    if (peerConnectionRef.current) {
      console.warn(
        "⚠️ Đã có PeerConnection hiện tại, không thể bắt đầu cuộc gọi mới"
      );
      return;
    }
    try {
        ...
        switch (signal.type) {
          case "offer":
            if (pc.signalingState !== "stable") {
              console.warn(
                "⚠️ Nhận offer khi signalingState không phải stable:",
                pc.signalingState
              );
              return;
            }
  ...
  return (
    <Modal
      visible={show}
      onRequestClose={() => endCall(false)} // Dùng onRequestClose thay vì onHide
      transparent
      animationType="slide"
    >
